    ...
    <table class="table">
        ...
        @foreach ($qrcodes as $qrcode)
            <tr>
                ...
                <td>
                    <a href="{{ url("storage/qrcodes/{$qrcode->id}.png") }}" download>
                        <i class="fa-solid fa-qrcode" style="font-size: 30px"></i>
                    </a>
                    {{-- <a href="{{ url("storage/{$qrcode->logo}") }}">
                        imgcenter
                    </a> --}}
                </td>
                <td><a href="{{ route('qrcodes.show',['qrcode' => $qrcode->id ]) }}" class="btn btn-info"       style="width: 100%"><i class="fa-solid fa-magnifying-glass"></i></a></td>
                <td><a href="{{ route('qrcodes.edit',['qrcode' => $qrcode->id ]) }}" class="btn btn-secondary"  style="width: 100%"><i class="fa-solid fa-pen-to-square"></i> </a></td>
                <td>
                    <form action="{{ route('qrcodes.destroy',['qrcode' => $qrcode->id ]) }}" method="POST">
                        ...
                        <button type="submit" class="btn btn-danger" style="width: 100%"><i class="fa-solid fa-trash-can"> </i></button>
                    </form>
                </td>
            </tr>
        @endforeach
